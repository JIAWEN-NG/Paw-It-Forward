...
// Function to edit a donation by ID
const editDonation = async (req, res) => {
    const { id, itemCategory, condition, petType, itemsDonated, location } = req.body; // Include location in request body

    try {
        if (!id || !itemCategory || !condition || !petType || !itemsDonated || !location) {
            return res.status(400).json({ message: 'Missing required fields' });
        }
